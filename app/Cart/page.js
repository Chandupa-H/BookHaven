...
//               {/* Checkout Button */}
//               <button className="w-full bg-blue-600 text-white py-4 rounded-lg font-semibold hover:bg-blue-700 transition-colors flex items-center justify-center gap-2 mb-4">
...
// import Navbar from "../Components/Navbar";
// import Footer from "../Components/Footer";
// import { setCart } from "../store/slices/cartSlice"; // Adjust path as needed
...
const CartPage = () => {
  ...
  const books = useSelector((state) => state.books.items || []); // Adjust based on your book slice structure
  ...
  const enrichedCartItems = useMemo(() => {
    return cartItems
      .map((cartItem) => {
        const book = books.find((b) => b.id === cartItem.book);
        if (!book) return null;
